...
base_dt = datetime(2023, 6, 1, 10, 0, 0)  # Used for generating date/times
...
# ----------------------------
# 4. FINANCIALGOAL – 3NF
# Each investor gets between 1 and 3 goals (with random timeline).
# ----------------------------
...
# We do NOT store 'investedvalue' permanently here; we only store it internally to compute performance.
# We do store 'annualisedreturn' for now, which we'll correct after performance generation.
# We'll drop 'investedvalue' from the final CSV columns to match your 3NF design.
# ----------------------------
...
for row in fg_data:
    ...
    portfolio_counter += 1
    investedvalue = random.randint(50000, 450000)  # used for performance
...
# We'll then fix 'annualisedreturn' in portfolio.csv using the last row (month 12).
# ----------------------------
...
for pinfo in port_data:
    portfolioid = pinfo[0]
    invested = portfolio_invested_map[portfolioid]  # from our map
    latest_annualreturns = None
    for month in range(1, 13):
        dt_obj = random_datetime_in_month(2024, month)
        dt_str = dt_obj.strftime("%Y-%m-%d %H:%M:%S")
        gainloss = random.randint(-2000, 2000)
        fraction = (gainloss + 2000) / 4000.0  # Maps [-2000..2000] to [0..1]
        # annualreturns in range [-5..+15]%
    ...
    # Overwrite the placeholder annualisedreturn in the portfolio data
...
# ----------------------------
# 7. ASSET – 3NF
# Each portfolio gets 1 or 2 assets, summing to total allocation ratio = 1.0
# ----------------------------
...
# ----------------------------
# 8. FUNDS – 3NF
# Subclass of asset (first 40 asset IDs)
# ----------------------------
...
# ----------------------------
# 9. CASH – 3NF
# Subclass of asset (next 20 asset IDs)
# ----------------------------
...
# Guarantee a top-up on the 1st day of each month in 2024
for month in range(1, 13):
...
pt_data = []
# Generate a few random company IDs for the "brk" references
for i in range(1, 16):
...
combined_ra_data = []
for row in ra1_data:
    # row: [phonenumber, datetime, q1, q2, q3, q4, q5]
    key = tuple(row[2:7])
...
# Update the final 'annualisedreturn' by the last record from performance
# and save as repaired_portfolio.csv
# (Dropping 'annualisedreturn' then re-merging it from performance)
# ----------------------------
def repair_portfolio_csv():
    performance_df = pd.read_csv("performance.csv", parse_dates=["datetime"])
    portfolio_df = pd.read_csv("portfolio.csv")
    
    # Get the latest annual return for each portfolio
    latest_returns = (
    ...
    # Remove the existing 'annualisedreturn' column and merge with the new one
    portfolio_df = portfolio_df.drop(columns=["annualisedreturn"]).merge(
        latest_returns, on="portfolioid", how="left"
    )

    # 'investedvalue' was never in portfolio.csv to begin with (we omitted it),
    # so no need to drop that. Now we write the new file:
...
# Using a custom date parser if needed.
# If your performance.csv datetime is already 'YYYY-MM-DD HH:MM:SS',
# you can skip the custom parser or adapt as needed.
# ----------------------------
def split_performance_csv(filename):
    """
    Reads the performance CSV file and splits it into three separate DataFrames
    for performance1, performance2, and performance3, then saves each as a new CSV.
    ...
    """
    # If your date in performance.csv is already YYYY-MM-DD HH:MM:SS, we can parse directly:
    df = pd.read_csv(filename, parse_dates=["datetime"])

    # If you had a different format (e.g. dd/mm/yy), do something like:
    # df = pd.read_csv(filename, parse_dates=["datetime"], date_parser=lambda x: pd.to_datetime(x, format='%d/%m/%y %H:%M'))

    # Convert to a consistent format for PostgreSQL if needed:
...
# Now split the performance data:
split_performance_csv('performance.csv')

print("Split 'performance.csv' into performance1.csv, performance2.csv, performance3.csv.")
